...
export function LogoMarquee() {
  ...
  return (
    <section className="border-y border-border/60 bg-card/40 py-6 sm:py-8">
      <p className="container-px mx-auto mb-3 max-w-7xl text-center text-[10px] font-medium uppercase tracking-[0.15em] text-muted-foreground sm:mb-4 sm:text-xs sm:tracking-[0.2em]">
        Tutors from the world's most respected institutions
      </p>
      <div className="relative overflow-hidden">
        <div className="flex w-max animate-marquee items-center gap-6 whitespace-nowrap font-display text-sm font-bold text-muted-foreground/70 sm:gap-10 sm:text-lg md:text-xl">
          {row.map((n, i) => (
...
export function Stats() {
  ...
  return (
    <section className="container-px mx-auto max-w-7xl py-12 sm:py-16 md:py-20">
      <div className="grid grid-cols-2 gap-px overflow-hidden rounded-2xl bg-border sm:rounded-3xl md:grid-cols-4">
        {stats.map((s, i) => (
          ...
          >
            ...
            <div className="mt-1 text-xs text-muted-foreground sm:mt-2 sm:text-sm">{s.l}</div>
          </motion.div>
        ))}
      </div>
    </section>
...
export function Features() {
  ...
  return (
    <section id="features" className="container-px mx-auto max-w-7xl py-14 sm:py-20 md:py-24">
      <SectionHeader
      ...
      <div className="mt-10 grid gap-4 sm:mt-14 sm:gap-5 md:grid-cols-2 lg:grid-cols-3">
        {features.map((f, i) => (
          <motion.div
            ...
            className="group relative overflow-hidden rounded-2xl border border-border bg-card p-5 transition-all hover:-translate-y-1 hover:shadow-[var(--shadow-soft)] sm:rounded-3xl sm:p-7"
          >
            <div className="grid h-10 w-10 place-items-center rounded-xl bg-primary-soft text-xl sm:h-12 sm:w-12 sm:rounded-2xl sm:text-2xl">
              {f.icon}
            </div>
            <h3 className="mt-4 font-display text-lg font-bold sm:mt-5 sm:text-xl">{f.t}</h3>
...
export function Subjects() {
  ...
  return (
    <section id="subjects" className="container-px mx-auto max-w-7xl py-14 sm:py-20 md:py-24">
      <div className="flex flex-col items-start justify-between gap-4 sm:gap-6 md:flex-row md:items-end">
        ...
        <Link
          to="/subjects"
          className="inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-4 py-2 text-sm font-semibold hover:border-primary sm:px-5 sm:py-2.5"
        >
          See all subjects →
        </Link>
      </div>
      <div className="mt-10 grid grid-cols-2 gap-2 sm:mt-14 sm:gap-3 md:grid-cols-3 lg:grid-cols-4">
        {subjects.map((s, i) => (
          <motion.div
            key={s.n}
            {...fadeUp}
          ...
          >
            <Link
              ...
              className="group flex items-center justify-between rounded-xl border border-border bg-card p-3 transition-all hover:-translate-y-0.5 hover:border-primary hover:shadow-[var(--shadow-card)] sm:rounded-2xl sm:p-5"
            >
              <div>
                <div className="text-xl sm:text-2xl">{s.emoji}</div>
                <div className="mt-2 font-display text-sm font-bold sm:mt-3 sm:text-base">{s.n}</div>
                <div className="text-[10px] text-muted-foreground sm:text-xs">{s.c}</div>
              </div>
              <span className="hidden text-muted-foreground transition-all group-hover:translate-x-1 group-hover:text-primary sm:inline">
                →
              </span>
            </Link>
          </motion.div>
...
export function Tutors() {
  ...
  return (
    <section id="tutors" className="container-px mx-auto max-w-7xl py-14 sm:py-20 md:py-24">
      <div className="flex flex-col items-start justify-between gap-4 sm:gap-6 md:flex-row md:items-end">
        <SectionHeader
          align="left"
          eyebrow="Top tutors"
      ...
      <div className="mt-8 grid gap-4 sm:mt-12 sm:grid-cols-2 sm:gap-5 lg:grid-cols-4">
        {tutors.map((t, i) => (
          <motion.article
            ...
            {...fadeUp}
            transition={{ duration: 0.4, delay: i * 0.06 }}
            className="group overflow-hidden rounded-2xl border border-border bg-card transition-all hover:-translate-y-1 hover:shadow-[var(--shadow-soft)] sm:rounded-3xl"
          >
            ...
            <div className="p-4 sm:p-5">
              ...
              <div className="mt-3 flex items-center justify-between border-t border-border pt-3 sm:mt-4 sm:pt-4">
                ...
                <button className="rounded-full bg-foreground px-3 py-1.5 text-[11px] font-semibold text-background transition-colors group-hover:bg-primary group-hover:text-primary-foreground sm:px-4 sm:py-2 sm:text-xs">
                  Book
                </button>
              </div>
            </div>
...
export function HowItWorks() {
  ...
  return (
    <section id="how" className="relative overflow-hidden py-14 sm:py-20 md:py-24">
      <div className="bg-mesh absolute inset-0 -z-10 opacity-50" />
        ...
        <div className="mt-10 grid gap-4 sm:mt-16 sm:gap-6 md:grid-cols-3">
          {steps.map((s, i) => (
            <motion.div
              ...
              className="relative rounded-2xl border border-border bg-card p-5 sm:rounded-3xl sm:p-8"
            >
              <div className="font-display text-4xl font-extrabold text-primary/20 sm:text-5xl md:text-6xl">
                {s.n}
              </div>
              <h3 className="mt-1 font-display text-lg font-bold sm:mt-2 sm:text-xl md:text-2xl">
                {s.t}
              </h3>
...
export function Testimonials() {
  ...
  return (
    <section className="container-px mx-auto max-w-7xl py-14 sm:py-20 md:py-24">
      <SectionHeader
      ...
      <div className="mt-10 grid gap-4 sm:mt-14 sm:gap-5 md:grid-cols-2 lg:grid-cols-3">
        {items.map((t, i) => (
          <motion.figure
            ...
            {...fadeUp}
            transition={{ duration: 0.4, delay: (i % 3) * 0.06 }}
            className="rounded-2xl border border-border bg-card p-5 sm:rounded-3xl sm:p-7"
          >
            <div className="flex gap-0.5 text-sm text-primary sm:text-base">{"★★★★★"}</div>
            <blockquote className="mt-3 font-display text-base leading-snug sm:mt-4 sm:text-lg">
              &ldquo;{t.q}&rdquo;
            </blockquote>
            <figcaption className="mt-4 flex items-center gap-3 border-t border-border pt-3 sm:mt-5 sm:pt-4">
              <div className="grid h-8 w-8 place-items-center rounded-full bg-primary-soft text-sm font-bold text-primary sm:h-9 sm:w-9">
                {t.n[0]}
              </div>
              <div>
...
export function Pricing() {
  ...
  return (
    <section id="pricing" className="container-px mx-auto max-w-7xl py-14 sm:py-20 md:py-24">
      <SectionHeader
        eyebrow="Pricing"
      ...
      <div className="mt-10 grid gap-4 sm:mt-14 sm:gap-5 md:grid-cols-2 lg:grid-cols-3">
        {tiers.map((t, i) => (
          <motion.div
            ...
            {...fadeUp}
            transition={{ duration: 0.4, delay: i * 0.06 }}
            className={`relative rounded-2xl border p-5 sm:rounded-3xl sm:p-8 ${
              t.popular
          ...
          >
            {t.popular && (
              <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-primary px-3 py-1 text-[11px] font-bold uppercase tracking-wider text-primary-foreground sm:text-xs">
                Most popular
              </span>
            )}
            <h3 className="font-display text-base font-bold sm:text-lg">{t.name}</h3>
            <p
              className={`mt-1 text-xs sm:text-sm ${t.popular ? "text-background/70" : "text-muted-foreground"}`}
            >
              {t.desc}
            </p>
            <div className="mt-4 flex items-end gap-1 sm:mt-6">
              <span className="font-display text-3xl font-extrabold tracking-tight sm:text-5xl">
                {t.price}
              </span>
              {t.per && (
                <span
                  className={`mb-1 text-xs sm:mb-2 sm:text-sm ${t.popular ? "text-background/70" : "text-muted-foreground"}`}
                >
                  {t.per}
                </span>
              )}
            </div>
            <ul className="mt-5 space-y-2.5 text-sm sm:mt-7 sm:space-y-3">
              {t.features.map((f) => (
                <li key={f} className="flex items-start gap-2">
                  <span
                    className={`mt-0.5 grid h-4 w-4 shrink-0 place-items-center rounded-full text-[10px] sm:h-5 sm:w-5 sm:text-xs ${t.popular ? "bg-primary text-primary-foreground" : "bg-primary-soft text-primary"}`}
                  >
                    ✓
                  </span>
            ...
            <button
              className={`mt-6 w-full rounded-full px-5 py-2.5 text-sm font-semibold transition-transform hover:-translate-y-0.5 sm:mt-8 sm:py-3 ${
                t.popular ? "bg-primary text-primary-foreground" : "bg-foreground text-background"
              }`}
            >
              {t.cta}
            </button>
...
export function FAQ() {
  ...
  return (
    <section id="faq" className="container-px mx-auto max-w-3xl py-14 sm:py-20 md:py-24">
      <SectionHeader eyebrow="FAQ" title="Questions, answered." />
      <div className="mt-8 divide-y divide-border rounded-2xl border border-border bg-card sm:mt-12 sm:rounded-3xl">
        {items.map((it, i) => (
          <details key={i} className="group p-4 sm:p-6 [&_summary::-webkit-details-marker]:hidden">
            <summary className="flex cursor-pointer items-center justify-between gap-4 font-display text-sm font-semibold sm:text-lg">
              {it.q}
              <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-muted text-xs text-muted-foreground transition-transform group-open:rotate-45 sm:h-8 sm:w-8 sm:text-sm">
                +
              </span>
            </summary>
            <p className="mt-2 text-sm leading-relaxed text-muted-foreground sm:mt-3">{it.a}</p>
          </details>
        ))}
      </div>
    </section>
...
export function CTA() {
  return (
    <section id="cta" className="container-px mx-auto max-w-7xl py-12 sm:py-16 md:py-20">
      <div className="relative overflow-hidden rounded-2xl bg-foreground p-6 text-background sm:rounded-3xl sm:p-10 md:p-16">
        <div
          aria-hidden
          className="absolute -right-16 -top-16 h-48 w-48 rounded-full bg-primary/40 blur-2xl sm:-right-24 sm:-top-24 sm:h-72 sm:w-72 sm:blur-3xl"
        />
        <div
          aria-hidden
          className="absolute -bottom-16 -left-16 h-48 w-48 rounded-full bg-primary/30 blur-2xl sm:-bottom-24 sm:-left-24 sm:h-72 sm:w-72 sm:blur-3xl"
        />
        <div className="relative grid items-center gap-6 sm:gap-8 md:grid-cols-2">
          <div>
            ...
            <p className="mt-3 max-w-md text-sm text-background/70 sm:mt-4 sm:text-base">
          ...
          <form onSubmit={(e) => e.preventDefault()} className="flex flex-col gap-3 sm:flex-row">
            <input
              ...
              className="flex-1 rounded-full border border-background/20 bg-background/10 px-4 py-3 text-sm text-background placeholder:text-background/50 outline-none focus:border-primary sm:px-5 sm:py-3.5"
            />
            <button className="rounded-full bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground shadow-[var(--shadow-glow)] transition-transform hover:-translate-y-0.5 sm:px-6 sm:py-3.5">
              Start free →
            </button>
          </form>
        </div>
  ...
  return (
    <footer className="border-t border-border bg-card/40">
      <div className="container-px mx-auto max-w-7xl py-10 sm:py-16">
        <div className="grid gap-8 sm:grid-cols-2 sm:gap-10 md:grid-cols-[1.4fr_2fr]">
          <div>
            <div className="flex items-center gap-2.5">
              <div className="grid h-8 w-8 place-items-center rounded-lg bg-primary text-primary-foreground sm:h-9 sm:w-9 sm:rounded-xl">
            ...
            <p className="mt-3 max-w-sm text-xs text-muted-foreground sm:mt-4 sm:text-sm">
              Learn together, grow together. The world's most loved tutoring platform — built with
              care from India for the world.
            </p>
          </div>
        ...
        <div className="mt-8 flex flex-col items-start justify-between gap-3 border-t border-border pt-5 sm:mt-12 sm:flex-row sm:items-center sm:pt-6">
          ...
          <div className="flex gap-2 sm:gap-3">
            {["Twitter", "Instagram", "LinkedIn", "YouTube"].map((s) => (
              <a
                key={s}
                href="#"
                className="rounded-full border border-border bg-card px-2.5 py-1 text-[10px] hover:border-primary hover:text-foreground sm:px-3 sm:text-xs"
              >
                {s}
              </a>
            ))}
