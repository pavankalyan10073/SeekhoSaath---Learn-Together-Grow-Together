...
function SignupPage() {
  ...
  const handleTutorSubmit = async (step1Data: SignupForm) => {
    ...
    try {
      ...
      const existing = JSON.parse(localStorage.getItem("seekhoSaath_tutorApplications") || "[]");
      existing.push(application);
      localStorage.setItem("seekhoSaath_tutorApplications", JSON.stringify(existing));
    ...
    <div className="relative min-h-screen bg-background">
      ...
      <main className="relative z-10 flex items-start justify-center px-4 pt-32 sm:pt-32 pb-8 min-h-screen">
        {step === "role" ? (
          ...
          <Card className="w-full max-w-5xl border-border shadow-[var(--shadow-card)]">
            <CardHeader className="text-center pb-2">
              <div className="mx-auto mb-3 grid h-12 w-12 place-items-center rounded-xl bg-primary-soft text-primary">
