...
export default async (req, res) => {
  ...
  if (req.method === "GET") {
    try {
      ...
      return sendJson(res, 500, { success: false, message: error.message || "Failed to fetch" });
    }
  }
  ...
  if (req.method === "PATCH") {
    try {
      ...
      if (body.action === "approve") {
        ...
        if (appError || !app) return sendJson(res, 404, { success: false, message: "Application not found" });
        ...
        return sendJson(res, 200, { success: true, data: { tutorId } });
      }

      if (body.action === "reject") {
        const reason = body.reason;
        if (!reason) return sendJson(res, 400, { success: false, message: "Rejection reason is required" });
      ...
      return sendJson(res, 500, { success: false, message: error.message || "Operation failed" });
    }
  }
